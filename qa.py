        ...
        def __init__(self, story):
            ...
            for sentence in self.sentences:
                ...
                for token in doc:
                    ...
                    spacyTuple.append(token.pos_)
                    spacyTuple.append(token.lemma_)
                    spacyList.append(spacyTuple)
                ...
                self.spacyLemma.append(spacyList)
    ...
    def __init__(self, question):
        ...
        for line in question:
            ...
            if line.startswith("Difficulty:"):
                self.difficulty = line.lstrip("Difficulty: ").strip()
        ...
        for token in doc:
            ...
            self.spacyLemma.append(spacyTuple)
        ...
        regex = re.compile("what is|what was|what did|what are|what does|what")
        if search(regex, question):
            self.type = "what"
            self.typeTag.add('PRODUCT')
            self.typeTag.add('WORK_OF_ART')
        ...
        if search(regex,question):
            ...
            self.typeTag.add('EVENT')
            self.typeTag.add('FAC')
...
def Overlap(story, question, bestSentenceMatch, bestSentenceLocation, matchingNERSentences):
    ...
    #print(question.spacyLemma)

    for sentence in matchingNERSentences:
        # Get the words in common between both sentence and question
        overlap = set([tuple((sl)) for sl in question.spacyLemma]) & set([tuple((sl)) for sl in story.spacyLemma[sentence]])
        # print(story.spacyLemma[sentence])
        # print("OVERLAP")
        # print(overlap)
        ...
        for ol in overlap:
            #score matching non nouns higher
            if(ol[0] == 'NUM' or ol[0] == 'VERB' or ol[0] == 'ADJ'):
                score+= 4
            elif(ol[0] == 'NOUN' and ol[0] == 'PROPN'):
                score+=2
            elif(ol[0] == 'NOUN'):
                score+=1
            else:
        ...
        if score in bestSentenceMatch.keys():
            ...
            bestSentenceLocation[score].append(location)
        # Add the overlap to the dictionary
        else:
            bestSentenceMatch[score] = [sentence]
            bestSentenceLocation[score] = [location]
...
def FindAnswer(question, story, bestSentenceMatch):
    ...
    questionLemmas = list(question.lemma)
    validSentences = []
    foundSentence = False
    # for bsm in bestSentenceMatch:
    #     print(story.ner[bsm])
    ...
    WriteToFile(question, questionNERTag, questionLemmas, bestSentence, story)            
...
def DirectMatch(question, story):
    ...
    return answer
def WriteToFile(question, questionNERTag,questionLemmas,  sentence, story):
    ...
    if sentence != -1:
        for tag in story.ner[sentence]:
            if tag[1] in questionNERTag:
                if tag[0].lower() not in questionLemmas:
                    #print(questionLemmas)
                    #print(tag[0])
                    answer += tag[0] + ' '
    ...
    writeOrAppend = ''
    #storyID = qidClean[0:10]
    #fileName = str(storyID) + ".response"
    # print(line1)
    # print(line2)
    # print(line3)
    # print(line4)
